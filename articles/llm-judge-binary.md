...
$\widehat{Y} \in \{ 0,1\}$ as a binary surrogate for the human label
$Y$, and we plan calibration size and power for the EIF estimator.

Assume we have estimates:

- Prevalence $p = P(Y = 1)$ from pilot human labels
- Sensitivity $q_{1} = P\left( \widehat{Y} = 1 \mid Y = 1 \right)$
- Specificity $q_{0} = P\left( \widehat{Y} = 0 \mid Y = 0 \right)$
...
Suppose we want $80\%$ power to detect a $\Delta = 0.05$ shift in win
rate.
...
With the above numbers, $\approx 550$ labeled comparisons are needed for
$80\%$ power; using only 100 or 250 labeled comparisons delivers much
lower power:
...
If you already have
${\widehat{\mu}}_{0} = E\left\lbrack Y \mid \widehat{Y} = 0 \right\rbrack$,
${\widehat{\mu}}_{1} = E\left\lbrack Y \mid \widehat{Y} = 1 \right\rbrack$,
and $p_{\widehat{Y}}$, you can pass them directly:
